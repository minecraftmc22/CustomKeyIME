...
- **⚙ 设置**：键盘高度 / 背景透明度 / 背景明暗 / **自定义背景图片**，保存即时生效

### 主题
- **Material You 3**：Android 12+ 读取系统动态色，低版本回退 M3 深色基线
- 键盘背景支持**透明度**与**明暗**调节
- 支持**导入自定义背景图片**（设置页 → 选择图片，透明度 / 明暗同样作用于图片；
  用系统文件选择器读取并申请长期权限，重启后依然有效）
...
- ✅ **导出 / 导入**：「⤓ 导出自定义按键」与「⤒ 导入自定义按键」
  - 导出：保存为 **JSON 文件**（系统文件选择器，无需存储权限）/ **分享为文本**（可读）/
    **分享为 JSON**（对方可直接导入）/ **复制 JSON 到剪贴板**
  - 导入：**从文件导入**（.json）或 **从剪贴板导入**（JSON 文本），支持
    **合并到现有**（同名分组追加，自动跳过重复按键）或 **替换全部**
...
│   │   ├── KeyBackup.java               # 自定义按键导出 / 导入（JSON 序列化与解析）
│   │   ├── KeysActivity.java            # 按键管理界面（分组 / 折叠 / 移动 / 增删改 / 导入导出）
│   │   ├── SettingsActivity.java        # 键盘设置（高度 / 透明度 / 明暗 / 背景图片）
...
```bash
git tag v1.8 && git push origin v1.8   # 触发 Release 打包
```
